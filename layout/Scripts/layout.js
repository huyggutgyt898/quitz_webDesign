...
function loadQuestionsPage(event) {
    ...
    if (questionIframe) questionIframe.style.display = 'block';
...
function loadHomePage(event) {
    ...
    if (questionIframe) questionIframe.style.display = 'none';
...
// NAVBAR ACTIVE STATE (chỉ cần 1 phần này)
document.addEventListener('DOMContentLoaded', function() {
    const navItems = document.querySelectorAll('.layout-navbar__item');
    
    navItems.forEach(item => {
        item.addEventListener('click', function(e) {
            // Nếu item có onclick, không xử lý thêm
    ...
    // Khởi tạo active state dựa trên URL hoặc mặc định
    const currentPath = window.location.pathname;
    if (currentPath.includes('showQuestion') || currentPath.includes('questions')) {
        updateActiveNav('questions');
    } else {
        updateActiveNav('home');
    }
